...
export const DetailsProvider = ({ children }) => {
    ...
    const getConsultas = async (id_medico = idLogado, id_paciente = Paciente?.userId) => {
        let { data: listaConsultas = [] } = await api.get(`consultas?id_medico=${id_medico}&id_paciente=${id_paciente}`);
    ...
    const salvarConsulta = async (dados) => {
        ...
        return {
            ...
            message: !!response?.data?.id ? "Consulta registrada!" : "Ops! Algo deu errado",
        };
    };
    ...
    const salvarDiagnostico = async (dados) => {
        ...
        return {
            status: !!response?.data?.id ? true : false,
            message: !!response?.data?.id ? "incluido com sucesso!" : "Ops! Algo deu errado",
        };
    };
