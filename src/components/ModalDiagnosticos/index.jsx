...
import { Button, ColumnBox, ModalBox, RowBox, Text, Input, Select, FormBox, Datagrid } from "../../theme";
        ...
        <ModalBox hidden={stateModalDiagnosticos}>
            <ColumnBox bgColor="grey0" width="60%" height="100%">
                <RowBox bgColor="primary" style={rowBar}>
                    ...
                    <Button bgColor="negative" style={button} onClick={() => Switch("ModalDiagnosticos")}>
                        x
                    </Button>
                </RowBox>
                <FormBox onSubmit={handleSubmit(novaDiagnostico)}>
                    <RowBox>
                        <Select errorMsg={errors?.tipo?.message} label="Tipo" register={register} name="tipo" options={optionsTipo}></Select>
                        <Input errorMsg={errors?.descricao?.message} label="Descrição" register={register} name="descricao"></Input>
                    </RowBox>
                    <RowBox>
                        <Input errorMsg={errors?.data_inicio?.message} label="Data" register={register} name="data_inicio" type="date" defaultValue={dataAtual}></Input>
                        <Input label="Data Final" register={register} name="data_fim" type="date"></Input>
                ...
                <ColumnBox overflow="auto" height="100%">
                    <Datagrid height="100%" options={{ showHeader: false }} bgColor="grey0" columns={colunas} data={diagnosticos} />
                </ColumnBox>
            </ColumnBox>
        </ModalBox>
    );
